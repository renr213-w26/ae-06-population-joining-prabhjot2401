...
# add code here
population_continet<-population|>left_join(continents,by=join_by(country==entity))
```
...
population_continet|>filter(is.na(continent))
```
...
ggplot(population_summary, aes(x = continent, y = total_pop)) +
  ...
  labs(
    ...
    title = "Total Population by Continent, 2022"
  )
```
